...
object Bp2Response {
    ...
    class RtParam(var bytes: ByteArray) {
        ...
        init {
            ...
            index++
            batteryVol = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            // reserve 4
        }
    ...
    class RtWave(var bytes: ByteArray) {
        ...
        init {
            ...
            when (type) {
                ...
                DataTypeBpResult -> dataBpResult =
                    DataBpResult(bytes.copyOfRange(index, index + 20))

                DataTypeEcging -> dataEcging = DataEcging(bytes.copyOfRange(index, index + 20))
                DataTypeEcgResult -> dataEcgResult =
                    DataEcgResult(bytes.copyOfRange(index, index + 20))
            }
            index += 20
            if (bytes.size > 21) {
                waveLen = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
                index += 2
                if (waveLen != 0) {
                    wave = bytes.copyOfRange(index, index + (2 * waveLen))
                    waveFs = FloatArray(waveLen)
                    for (i in 0 until waveLen) {
                        waveFs!![i] = BpDataController.byteTomV(wave!![2 * i], wave!![2 * i + 1])
                    }
                }
            } else {
                waveLen = 0
    ...
    class DataBping(var bytes: ByteArray) {
        ...
        init {
            ...
            index++
            pressure = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            isPulseDetect = bytes[index] == 0x01.toByte()
            index++
            pr = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            // reserve 14
        }
    ...
    class DataBpResult(var bytes: ByteArray) {
        ...
        init {
            ...
            pressure = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            sys = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            dia = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            mean = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            pr = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            state = bytes[index]
    ...
    class DataEcging(var bytes: ByteArray) {
        ...
        init {
            var index = 0
            duration = toUInt(bytes.copyOfRange(index, index + 4)).toInt()
            index += 4
            status = bytes.copyOfRange(index, index + 4)
            index += 4
            hr = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            // reserve 10
        }
    ...
    class DataEcgResult(var bytes: ByteArray) {
        ...
        init {
            var index = 0
            result = toUInt(bytes.copyOfRange(index, index + 4)).toInt()
            index += 4
            hr = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            qrs = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            pvcs = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            qtc = toUInt(bytes.copyOfRange(index, index + 2)).toInt()
            index += 2
            // reserve 8
        }
